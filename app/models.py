...
class User(db.Model):
    ...
    date_registered = db.Column(db.Date, nullable=False, default=datetime.utcnow().date)
    time_registered = db.Column(db.Time, nullable=False, default=datetime.utcnow().time)
...
def new_user(email, dob, username, qual, password):
    new_user = User(
        ...
        qualification=qual,
        password=password,
        date_registered=datetime.utcnow().date(),
        time_registered=datetime.utcnow().time()
    )
...
class Marks(db.Model):
    ...
    def __repr__(self):
        return f'<Marks email={self.email} quiz_id={self.quiz_id} marks={self.marks}>'

# Function to add marks using email
def add_marks(email, quiz_id, marks):
    new_marks = Marks(
        email=email,
        quiz_id=quiz_id,
        marks=marks,
    ...
    db.session.commit()
    return f"Marks added: Email {email}, Quiz {quiz_id}, Marks {marks}"

# Function to get marks by email
def get_marks_by_email(email):
    marks = Marks.query.filter_by(email=email).all()
    return [
        [mark.email, mark.quiz_id, mark.marks, mark.date.strftime('%Y-%m-%d'), mark.time.strftime('%H:%M:%S')]
        for mark in marks
    ]